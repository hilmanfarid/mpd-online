...
define("FileName", "p_region_tree.php");
...
$query = "select p_region_id, NVL (parent_id, 0) parent_id, "
		. "upper(REPLACE(region_name, '''', '`')) region_name "
		. " FROM p_region START WITH "
		. "parent_id IS NULL CONNECT BY PRIOR p_region_id = parent_id";
        ...
        <tbody>
            <tr>
                <td class="CobaltHeaderTD" style="color: #696969;font-size: 11pt;font-weight: bold;font-family:Calibri,Arial,Tahoma;>
                    <span class="style1">&nbsp;<b>REGIONAL</b></span></td>
            </tr>
            <tr>
...
var TREE_ITEMS =
 [
  ['HOME (ROOT)', 'p_region.php?parent_id=0&retree=1'
<?php
	...
	while ($dbConn->next_record()) {
		if ($dbConn->f("p_region_id")!=$bdmnid) {
			$bdmnid=$dbConn->f("p_region_id");
			...
			$fileName = "p_region.php";
			$nplevel = $dbConn->f("p_region_id");
			echo "['" . $dbConn->f("region_name");
			if ($fileName=="") {
				echo "',''";
			} else {
				echo "','p_region.php?retree=1&parent_id=".$dbConn->f("p_region_id")."'";
			}
		}
	}
	while ($level>0) {
