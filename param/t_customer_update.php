<?php
//Include Common Files @1-2EF24515
define("RelativePath", "..");
define("PathToCurrentPage", "/param/");
define("FileName", "t_customer_update.php");
...
class clsGridt_customer_updateGrid { //t_customer_updateGrid class @2-216846CC
...
//Class_Initialize Event @2-CC6B45FC
    function clsGridt_customer_updateGrid($RelativePath, & $Parent)
    {
        ...
        $this->ComponentName = "t_customer_updateGrid";
        $this->Visible = True;
        $this->Parent = & $Parent;
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Grid t_customer_updateGrid";
        $this->Attributes = new clsAttributes($this->ComponentName . ":");
        $this->DataSource = new clst_customer_updateGridDataSource($this);
        ...
        $this->DLink->Page = "t_customer_update.php";
...
} //End t_customer_updateGrid Class @2-FCB6E20C
...
//Open Method @2-48E7C419
    function Open()
    {
        ...
        "                        and upper(x.npwd) like upper('%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%')\n" .
        "                        and upper(x.wp_name) like upper('%" . $this->SQLValue($this->wp->GetDBValue("3"), ccsText) . "%')\n" .
        "                        and upper(x.company_name) like upper('%" . $this->SQLValue($this->wp->GetDBValue("4"), ccsText) . "%')\n" .
        "                        and upper(x.company_brand) like upper('%" . $this->SQLValue($this->wp->GetDBValue("5"), ccsText) . "%')\n" .
        "                  )) cnt";
        $this->SQL = "select *\n" .
        "from t_customer a \n" .
        "where (upper(a.company_owner) like upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') \n" .
...
} //End t_customer_updateGridDataSource Class @2-FCB6E20C

class clsRecordt_customer_updateSearch { //t_customer_updateSearch Class @3-6B26D37F
...
//Class_Initialize Event @3-AC4D6DE4
    function clsRecordt_customer_updateSearch($RelativePath, & $Parent)
    {
        ...
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Record t_customer_updateSearch/Error";
        $this->ReadAllowed = true;
        if($this->Visible)
        {
            $this->ComponentName = "t_customer_updateSearch";
            $this->Attributes = new clsAttributes($this->ComponentName . ":");
...
//Operation Method @3-3315698A
    function Operation()
    {
        ...
        $Redirect = "t_customer_update.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "t_customer_update.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
} //End t_customer_updateSearch Class @3-FCB6E20C
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "t_customer_update.html";
$BlockToParse = "main";
...
//Include events file @1-9C842665
include_once("./t_customer_update_events.php");
...
// Controls
$t_customer_updateGrid = & new clsGridt_customer_updateGrid("", $MainPage);
$t_customer_updateSearch = & new clsRecordt_customer_updateSearch("", $MainPage);
$t_customer_updateForm = & new clsRecordt_customer_updateForm("", $MainPage);
$MainPage->t_customer_updateGrid = & $t_customer_updateGrid;
$MainPage->t_customer_updateSearch = & $t_customer_updateSearch;
$MainPage->t_customer_updateForm = & $t_customer_updateForm;
$t_customer_updateGrid->Initialize();
...
//Execute Components @1-65AFD959
$t_customer_updateSearch->Operation();
$t_customer_updateForm->Operation();
//End Execute Components

//Go to destination page @1-AF78CE5A
if($Redirect)
{
    $CCSEventResult = CCGetEvent($CCSEvents, "BeforeUnload", $MainPage);
    $DBConnSIKP->close();
    header("Location: " . $Redirect);
    unset($t_customer_updateGrid);
    unset($t_customer_updateSearch);
...
//Show Page @1-2AB2DB6B
$t_customer_updateGrid->Show();
$t_customer_updateSearch->Show();
