<?php
//Include Common Files @1-D0326056
define("RelativePath", "..");
define("PathToCurrentPage", "/lov/");
define("FileName", "lov_region_kec_upp.php");
...
//Show Method @2-3F737CA1
    function Show()
    {
        ...
        $this->DataSource->Parameters["expr41"] = 5;
...
//Prepare Method @2-2C042A70
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "expr41", ccsFloat, "", "", $this->Parameters["expr41"], "", false);
        $this->wp->AddParameter("2", "urls_keyword", ccsText, "", "", $this->Parameters["urls_keyword"], "", false);
        $this->wp->AddParameter("3", "urlp_business_area_id", ccsFloat, "", "", $this->Parameters["urlp_business_area_id"], 0, false);
    }
//End Prepare Method

//Open Method @2-A52D6BEB
    function Open()
    {
        ...
        "AND ( upper(region_name) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%'\n" .
        "OR upper(description) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%' )\n" .
        "AND p_business_area_id = " . $this->SQLValue($this->wp->GetDBValue("3"), ccsFloat) . " ) cnt";
        $this->SQL = "SELECT * \n" .
        "FROM p_region\n" .
        "WHERE p_region_level_id = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsFloat) . "\n" .
        "AND ( upper(region_name) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%'\n" .
        "OR upper(description) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%' )\n" .
        "AND p_business_area_id = " . $this->SQLValue($this->wp->GetDBValue("3"), ccsFloat) . " ";
...
//Operation Method @3-7D1C342D
    function Operation()
    {
        ...
        $Redirect = "lov_region_kec_upp.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "lov_region_kec_upp.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "lov_region_kec_upp.html";
$BlockToParse = "main";
...
//Include events file @1-00AD7795
include_once("./lov_region_kec_upp_events.php");
