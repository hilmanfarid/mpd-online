<?php
//Include Common Files @1-BB0375AB
define("RelativePath", "..");
define("PathToCurrentPage", "/lov/");
define("FileName", "lov_finance_period_new.php");
...
//Class_Initialize Event @2-13A1082C
    function clsGridLOV_ORDER($RelativePath, & $Parent)
    {
        ...
        $this->p_finance_period_id = & new clsControl(ccsTextBox, "p_finance_period_id", "p_finance_period_id", ccsText, "", CCGetRequestParam("p_finance_period_id", ccsGet, NULL), $this);
        $this->p_finance_period_id->Visible = false;
...
//DataSource Variables @2-A5EE6267
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-1BC1823B
...
//Open Method @2-E466A4AF
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (select *, to_char(start_date, 'dd-mm-yyyy') as start_date_txt, to_char(end_date, 'dd-mm-yyyy') as end_date_txt\n" .
        "from p_finance_period\n" .
        "where p_year_period_id = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsFloat) . " and\n" .
        "upper(code) like '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') cnt";
        $this->SQL = "select *, to_char(start_date, 'dd-mm-yyyy') as start_date_txt, to_char(end_date, 'dd-mm-yyyy') as end_date_txt\n" .
        "from p_finance_period\n" .
        "where p_year_period_id = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsFloat) . " and\n" .
        "upper(code) like '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' {SQL_OrderBy}";
...
//SetValues Method @2-BCD699AA
    function SetValues()
    {
        $this->code->SetDBValue($this->f("code"));
        $this->description->SetDBValue($this->f("description"));
        $this->p_finance_period_id->SetDBValue($this->f("p_finance_period_id"));
        $this->start_date->SetDBValue($this->f("start_date_txt"));
        $this->end_date->SetDBValue($this->f("end_date_txt"));
    }
...
//Class_Initialize Event @3-48C268AD
    function clsRecordLOV($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->p_year_period_id = & new clsControl(ccsTextBox, "p_year_period_id", "p_year_period_id", ccsText, "", CCGetRequestParam("p_year_period_id", $Method, NULL), $this);
            $this->p_year_period_id->Visible = false;
        }
    }
//End Class_Initialize Event

//Validate Method @3-5E8F1DF9
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
...
//Operation Method @3-F9CD562F
    function Operation()
    {
        ...
        $Redirect = "lov_finance_period_new.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "lov_finance_period_new.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "lov_finance_period_new.html";
$BlockToParse = "main";
...
//Include events file @1-A4932FB0
include_once("./lov_finance_period_new_events.php");
