<?php
//Include Common Files @1-F231E185
define("RelativePath", "..");
define("PathToCurrentPage", "/lov/");
define("FileName", "lov_coa.php");
...
//Class_Initialize Event @2-5B50EBC5
    function clsGridLovGrid($RelativePath, & $Parent)
    {
        ...
        $this->coa_id = & new clsControl(ccsTextBox, "coa_id", "coa_id", ccsFloat, "", CCGetRequestParam("coa_id", ccsGet, NULL), $this);
        $this->coa_id->Visible = false;
        $this->coa_name = & new clsControl(ccsLabel, "coa_name", "coa_name", ccsText, "", CCGetRequestParam("coa_name", ccsGet, NULL), $this);
...
//Show Method @2-ECA8FFC2
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            $this->ControlsVisible["coa_id"] = $this->coa_id->Visible;
            $this->ControlsVisible["coa_name"] = $this->coa_name->Visible;
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->coa_id->SetValue($this->DataSource->coa_id->GetValue());
...
//GetErrors Method @2-85EFE566
    function GetErrors()
    {
        $errors = "";
        $errors = ComposeStrings($errors, $this->coa_code->Errors->ToString());
        $errors = ComposeStrings($errors, $this->PILIH->Errors->ToString());
        $errors = ComposeStrings($errors, $this->coa_id->Errors->ToString());
        $errors = ComposeStrings($errors, $this->coa_name->Errors->ToString());
...
//DataSource Variables @2-503033FC
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-38B17F01
    function clsLovGridDataSource(& $Parent)
    {
        ...
        $this->coa_id = new clsField("coa_id", ccsFloat, "");
...
//Open Method @2-7F89EB53
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT * \n" .
        "FROM coa\n" .
        "WHERE upper(coa_code) LIKE upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR upper(coa_name) LIKE upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')) cnt";
        $this->SQL = "SELECT * \n" .
        "FROM coa\n" .
        "WHERE upper(coa_code) LIKE upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')\n" .
        "OR upper(coa_name) LIKE upper('%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')  {SQL_OrderBy}";
...
//SetValues Method @2-19B0E123
    function SetValues()
    {
        $this->coa_code->SetDBValue($this->f("coa_code"));
        $this->coa_id->SetDBValue(trim($this->f("coa_id")));
...
//Operation Method @3-0D60C12A
    function Operation()
    {
        ...
        $Redirect = "lov_coa.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "lov_coa.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
//Include events file @1-6FF8B872
include_once("./lov_coa_events.php");
