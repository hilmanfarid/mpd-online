...

{
...

  {
    ...
    var w_left = Math.ceil(screen.width/2-130);

    var w_top = Math.ceil(screen.height/2-95);

    top.newWinDatePickerObject = window.open(datepickerURL+"?random="+Math.random(), "DatePickerWindow", "dependent=yes,left="+w_left+",top="+w_top+",width=260,height=190,screenX=200,screenY=300,titlebar=no, center: yes, help: no, resizable: no, status: no");
...

{
...

  {
    ...
    imgPrev10Year = DatePickerObject.themePath+"Images/BBack.gif";
    ...
    imgNext10Year = DatePickerObject.themePath+"Images/Forward.gif";
  ...
  datepickerDocument += 
      ...
      "HREF=\"javascript:parent.opener.setPreviousYear()\"><IMG ALT=\"Go To Previous Year\" SRC=\""+imgPrevYear+"\" BORDER=\"0\"></A><A " +

      "HREF=\"javascript:parent.opener.setPreviousMonth()\"><IMG ALT=\"Go To Previous Month\"SRC=\""+imgPrevMonth+"\" BORDER=\"0\"></A>";
  ...
  "<select onchange=\"javascript:parent.opener.setManualMonth(this.value)\" id=\"labelMonth\" style=\"font-size: 11px; font-weight: bold\">";



  for (var i_bdr = 0; i_bdr < 12; i_bdr++)

  {

    if (i_bdr==DatePickerObject.currentMonth) {

       datepickerDocument += 

       "<option selected value=\""+i_bdr+"\">"+listMonths[i_bdr]+"</option>";

    } else {

       datepickerDocument += 

       "<option value=\""+i_bdr+"\">"+listMonths[i_bdr]+"</option>";
  ...
  "<select onchange=\"javascript:parent.opener.setManualYear(this.value)\" id=\"labelYear\" style=\"font-size: 11px; font-weight: bold\">";



  for (var i_bdr = year+30; i_bdr > year-100; i_bdr--)

  {

    if (i_bdr==DatePickerObject.currentYear) {

       datepickerDocument += 

       "<option selected value=\""+i_bdr+"\">"+i_bdr+"</option>";

    } else {

       datepickerDocument += 

       "<option value=\""+i_bdr+"\">"+i_bdr+"</option>";
  ...
  datepickerDocument += 
      ...
      "HREF=\"javascript:parent.opener.setNextMonth()\"><IMG ALT=\"Go To Next Month\"SRC=\""+imgNextMonth+"\" BORDER=\"0\"></A><A " +

      "HREF=\"javascript:parent.opener.setNextYear()\"><IMG ALT=\"Go To Next Year\"SRC=\""+imgNextYear+"\" BORDER=\"0\"></A>";
...

{
  ...
  var datepickerDocument = (isNav?"<HTML>" + "<HEAD>" +
      ...
      "<BODY leftmargin='0' topmargin='0' marginheight='0' marginwidth='0' ONUNLOAD='parent.opener.UnLoad();'>":"") + DatePickerBegin.replace(/{datepickerDocumentTop}/, datepickerDocumentTop);
...

{
...

  {
    ...
    top.newWinDatePickerObject = window.open(datepickerURL+"?random="+Math.random(), "DatePickerWindow", "dependent=yes,left="+w_left+",top="+w_top+",width=260,height=190,screenX=200,screenY=300,titlebar=yes, center: yes, help: no, resizable: yes, status: no");
...
//function setManualYear(lYear,newYear)

function setManualYear(newYear)
...
function setManualMonth(newMonth)

{
  ...
  DatePickerObject.currentMonth = month-0;
...

{
  ...
  DatePickerBegin =
      ...
      "<BODY leftmargin='0' topmargin='0' marginheight='0' marginwidth='0' ONUNLOAD='parent.opener.UnLoad();'>") + "<CENTER>";
  ...
  if (DatePickerObject.themeVersion=="3.0")
    ...
    DatePickerBegin += "<TABLE width='100%' CELLPADDING=0 CELLSPACING=0 BORDER=0><TR><TD>";

  else

    DatePickerBegin += "<TABLE width='100%' CELLPADDING=0 CELLSPACING=0 BORDER=0><TR><TD>{datepickerDocumentTop}</TD></TR><TR><TD ALIGN=CENTER VALIGN=TOP>";
...

  {
    ...
    var imgClear = DatePickerObject.themePath+"Images/"+translation_language+"/ButtonClear.gif";

    DatePickerEnd += "<tr class=Footer><form id=calTable name=calTable><td colspan=7><input ALT=\"Set Field to Today\" type=image src="+imgToday+" name=today onClick=\"parent.opener.setToday()\"><input ALT=\"Clear Field\" type=image src="+imgClear+" name=clear onClick=\"parent.opener.returnClear()\"></td></form></tr>";
  ...
  if (DatePickerObject.themeVersion!="3.0")
    ...
    DatePickerEnd +=
      ...
      "<INPUT TYPE='button' CLASS='CalendarButton' NAME='clear' VALUE='Clear' onClick=\"parent.opener.returnClear()\">" + 
