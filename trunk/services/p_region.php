...
$result = $dbConn->query("SELECT count(*) as count FROM p_region
						where (p_region_level_id = 3 or p_region_level_id = 4) and region_name ilike '%$searchTerm%'");
$dbConn->next_record();
$row = $dbConn->Record;
$count = $row['count'];
...
if($total_pages!=0) $SQL = "SELECT * FROM p_region
						where (p_region_level_id = 3 or p_region_level_id = 4) and region_name ilike '%$searchTerm%' ORDER BY $sidx $sord OFFSET $start LIMIT $limit";
else $SQL = "SELECT * FROM p_region
						where (p_region_level_id = 3 or p_region_level_id = 4) and region_name ilike '%$searchTerm%'";
