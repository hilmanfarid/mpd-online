...
define("FileName", "cetak_formulir_hotel_pdf.php");
...
$query="begin;";
$dbConn->query($query);
$query="select * from f_cetak_pajak_hotel(".$custId.", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "
...
while ($dbConn->next_record()) {
		...
		$data["o_hotel_grade_id"] = $dbConn->f("o_hotel_grade_id");		
...
class FormCetak extends FPDF {
	...
	*/
	...
	function PageCetak($data,$dataArr,$dataBaru) {
		...
		$this->Ln();
		$this->Cell($lengthCell, $this->height, "WAJIB PAJAK HOTEL", "LR", 0, 'C');
		$this->Ln();
		...
		*/
		...
		$this->Cell($per2+$per3, $this->height, "Setelah Formulir Pendaftaran ini diisi dan ditanda tangani,  harap diserahkan kembali Kepada Dinas Palayanan Pajak", "BR", 0, 'L');
		$this->Ln();
		...
		$this->Cell($per2, $this->height, "Pendataan Pajak Hotel", "TB", 0, 'L');
		$this->Cell($per3, $this->height, "", "TBR", 0, 'L');
		$this->Ln();
		...
		$this->Cell($kol8, $this->height, "", "RT", 0, 'C');
		$this->Ln();
		...
		$this->Cell($kol8, $this->height*2, "", "R", 0, 'C');
		$this->Ln($this->height-5);
		$this->Cell($kol1, $this->height*2, "", "L", 0, 'C');
		$this->Cell($kol2, $this->height*2, "Kelas Hotel", 0, 0, 'C');
		$this->Cell($kol3, $this->height+2, "Golongan", 0, 0, 'C');
		$this->Cell($kol4, $this->height+2, "Jumlah", 0, 0, 'C');
		$this->Cell($kol5, $this->height+2, "Frekuensi Pengguna", 0, 0, 'C');
		$this->Cell($kol6, $this->height*2, "Tarif Kamar", 0, 0, 'C');
		$this->Cell($kol7, $this->height*2, "Tarif Kamar", 0, 0, 'C');
		$this->Cell($kol8, $this->height*2, "", "R", 0, 'C');
		$this->Ln($this->height);
		$this->Cell($kol1, $this->height, "", "L", 0, 'C');
		$this->Cell($kol2, $this->height, "", 0, 0, 'C');
		$this->Cell($kol3, $this->height-2, "Kamar", 0, 0, 'C');
		$this->Cell($kol4, $this->height-2, "Kamar", 0, 0, 'C');
		$this->Cell($kol5, $this->height-2, "Layanan", 0, 0, 'C');
		$this->Cell($kol6, $this->height, "", 0, 0, 'C');
		$this->Cell($kol7, $this->height, "", 0, 0, 'C');
		$this->Cell($kol8, $this->height, "", "R", 0, 'C');
		$this->Ln();
		
		//isi kolom
		$this->SetWidths(array($kol1, $kol2, $kol3, $kol4, $kol5, $kol6, $kol7, $kol8));
		$this->SetAligns(array("C", "L", "L", "C", "C", "R", "R", "C"));
		for ($i=0; $i<count($dataBaru['room_qty']); $i++) {
		$this->RowMultiBorderWithHeight(array("",
											  $dataBaru["grade_name"],
											  $dataBaru["room_type_code"][$i],
											  $dataBaru["room_qty"][$i],
											  $dataBaru["service_qty"][$i],
											  $dataBaru["service_charge_wd"][$i],
											  $dataBaru["service_charge_we"][$i],
											  "")
											 ,
										array('L',
										      'TBL',
		...
		$this->Cell($kol8, $this->height, "", "RB", 0, 'C');
		$this->Ln();
