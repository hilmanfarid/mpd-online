...
$query="select 3 as sequence_no, to_char(sysdate,'dd-mm-yyyy') as letter_date_txt,  
	...
	* 
	...
	left join p_finance_period x on x.p_finance_period_id = ".$p_finance_period_id."
	left join p_vat_type y on y.p_vat_type_id = a.p_vat_type_id
...
while ($dbConn->next_record()) {
		$data[]= array(
			...
			'address' => $dbConn->f("wp_address_name"),
			...
			'debt_period_code' =>  $dbConn->f("periode"),
			'sequence_no' => $dbConn->f("sequence_no"),
			'letter_date_txt' => dateToString($dbConn->f("letter_date_txt")),
...
class FormCetak extends FPDF {
	...
	*/
	...
	function PageCetak($data,$no_urut) {
		...
		$this->Cell($lkepada2, $this->height, "Bandung,                             2014"/*.$data['letter_date_txt']*/, "R", 0, 'L');
		$this->Ln();
