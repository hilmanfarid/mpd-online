<?php
//Include Common Files @1-5C3819F0
define("RelativePath", "..");
define("PathToCurrentPage", "/trans/");
define("FileName", "t_message_outbox_bphtb.php");
...
class clsGridt_message_outboxGrid { //t_message_outboxGrid class @2-ED104005
...
//Class_Initialize Event @2-25CFEECB
    function clsGridt_message_outboxGrid($RelativePath, & $Parent)
    {
        global $FileName;
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->ComponentName = "t_message_outboxGrid";
        $this->Visible = True;
        $this->Parent = & $Parent;
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Grid t_message_outboxGrid";
        $this->Attributes = new clsAttributes($this->ComponentName . ":");
        $this->DataSource = new clst_message_outboxGridDataSource($this);
        ...
        $this->DLink->Page = "t_message_outbox_bphtb.php";
        $this->message_status = & new clsControl(ccsLabel, "message_status", "message_status", ccsText, "", CCGetRequestParam("message_status", ccsGet, NULL), $this);
        ...
        $this->t_message_outbox_id = & new clsControl(ccsHidden, "t_message_outbox_id", "t_message_outbox_id", ccsFloat, "", CCGetRequestParam("t_message_outbox_id", ccsGet, NULL), $this);
...
//Show Method @2-2877A283
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            $this->ControlsVisible["t_message_outbox_id"] = $this->t_message_outbox_id->Visible;
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->t_message_outbox_id->SetValue($this->DataSource->t_message_outbox_id->GetValue());
...
//GetErrors Method @2-825F4AEC
    function GetErrors()
    {
        ...
        $errors = ComposeStrings($errors, $this->t_message_outbox_id->Errors->ToString());
...
} //End t_message_outboxGrid Class @2-FCB6E20C

class clst_message_outboxGridDataSource extends clsDBConnSIKP {  //t_message_outboxGridDataSource Class @2-993D0EA6

//DataSource Variables @2-432BDBA8
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
    ...
    var $creation_date;
    var $message_type;
    var $t_message_outbox_id;
//End DataSource Variables

//DataSourceClass_Initialize Event @2-7639C9A8
    function clst_message_outboxGridDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Grid t_message_outboxGrid";
        $this->Initialize();
        ...
        $this->t_message_outbox_id = new clsField("t_message_outbox_id", ccsFloat, "");
        

    }
//End DataSourceClass_Initialize Event

//SetOrder Method @2-D6164F3A
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "outbox.creation_date DESC";
...
//Open Method @2-98C80D01
    function Open()
    {
        ...
        "	t_message_outbox outbox\n" .
        "LEFT JOIN sikp.p_message_type mtype ON mtype.p_message_type_id = outbox.p_message_type_id\n" .
        "where p_app_role_id_to = 17 {SQL_OrderBy}";
...
//SetValues Method @2-FBBF66ED
    function SetValues()
    {
        $this->message_status->SetDBValue($this->f("message_status"));
        $this->creation_date->SetDBValue($this->f("creation_date"));
        $this->message_type->SetDBValue($this->f("message_type"));
        $this->t_message_outbox_id->SetDBValue(trim($this->f("t_message_outbox_id")));
    }
//End SetValues Method

} //End t_message_outboxGridDataSource Class @2-FCB6E20C

class clsRecordt_message_outboxForm { //t_message_outboxForm Class @23-99286AD0
...
//Class_Initialize Event @23-D142D06C
    function clsRecordt_message_outboxForm($RelativePath, & $Parent)
    {
        ...
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Record t_message_outboxForm/Error";
        $this->DataSource = new clst_message_outboxFormDataSource($this);
        $this->ds = & $this->DataSource;
        ...
        if($this->Visible)
        {
            $this->ComponentName = "t_message_outboxForm";
            $this->Attributes = new clsAttributes($this->ComponentName . ":");
            ...
            $this->t_message_outbox_id = & new clsControl(ccsHidden, "t_message_outbox_id", "t_message_outbox_id", ccsFloat, "", CCGetRequestParam("t_message_outbox_id", $Method, NULL), $this);
            $this->Button_Insert1 = & new clsButton("Button_Insert1", $Method, $this);
        }
    }
//End Class_Initialize Event

//Initialize Method @23-CE1EC00C
    function Initialize()
    {
        ...
        $this->DataSource->Parameters["urlt_message_outbox_id"] = CCGetFromGet("t_message_outbox_id", NULL);
    }
//End Initialize Method

//Validate Method @23-14012CE3
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        ...
        $Validation = ($this->t_message_outbox_id->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        $Validation =  $Validation && ($this->message_type->Errors->Count() == 0);
        $Validation =  $Validation && ($this->t_message_outbox_id->Errors->Count() == 0);
        return (($this->Errors->Count() == 0) && $Validation);
    }
//End Validate Method

//CheckErrors Method @23-FA92F8C4
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->message_body->Errors->Count());
        $errors = ($errors || $this->message_type->Errors->Count());
        $errors = ($errors || $this->t_message_outbox_id->Errors->Count());
...
//Show Method @23-FB3CB69A
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->t_message_outbox_id->Errors->ToString());
...
} //End t_message_outboxForm Class @23-FCB6E20C

class clst_message_outboxFormDataSource extends clsDBConnSIKP {  //t_message_outboxFormDataSource Class @23-C60A9852

//DataSource Variables @23-AF194E99
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @23-B619D4B5
    function clst_message_outboxFormDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Record t_message_outboxForm/Error";
        $this->Initialize();
...
//Prepare Method @23-01FED08B
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlt_message_outbox_id", ccsFloat, "", "", $this->Parameters["urlt_message_outbox_id"], 0, false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
    }
//End Prepare Method

//Open Method @23-8E559F59
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT\n" .
        ...
        "	t_message_outbox outbox\n" .
        "LEFT JOIN sikp.p_message_type mtype ON mtype.p_message_type_id = outbox.p_message_type_id\n" .
        "where t_message_outbox_id = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsFloat) . "";
        $this->Order = "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->PageSize = 1;
        $this->query($this->OptimizeSQL(CCBuildSQL($this->SQL, $this->Where, $this->Order)));
...
} //End t_message_outboxFormDataSource Class @23-FCB6E20C
...
//Operation Method @3-62F2B6D6
    function Operation()
    {
        ...
        $Redirect = "t_message_outbox_bphtb.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "t_message_outbox_bphtb.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "button_submit", "button_submit_x", "button_submit_y", "Button1", "Button1_x", "Button1_y")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "t_message_outbox_bphtb.html";
$BlockToParse = "main";
...
//Include events file @1-214310F2
include_once("./t_message_outbox_bphtb_events.php");
...
// Controls
$t_message_outboxGrid = & new clsGridt_message_outboxGrid("", $MainPage);
$t_message_outboxForm = & new clsRecordt_message_outboxForm("", $MainPage);
$t_vat_setllementSearch = & new clsRecordt_vat_setllementSearch("", $MainPage);
$MainPage->t_message_outboxGrid = & $t_message_outboxGrid;
$MainPage->t_message_outboxForm = & $t_message_outboxForm;
$MainPage->t_vat_setllementSearch = & $t_vat_setllementSearch;
$t_message_outboxGrid->Initialize();
$t_message_outboxForm->Initialize();
...
//Execute Components @1-A2EBF49F
$t_message_outboxForm->Operation();
$t_vat_setllementSearch->Operation();
//End Execute Components

//Go to destination page @1-2D2EC2D5
if($Redirect)
{
    $CCSEventResult = CCGetEvent($CCSEvents, "BeforeUnload", $MainPage);
    $DBConnSIKP->close();
    header("Location: " . $Redirect);
    unset($t_message_outboxGrid);
    unset($t_message_outboxForm);
...
//Show Page @1-49C6F2A0
$t_message_outboxGrid->Show();
$t_message_outboxForm->Show();
