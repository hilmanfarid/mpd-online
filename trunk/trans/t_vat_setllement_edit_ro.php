<?php
//Include Common Files @1-A2E301A1
define("RelativePath", "..");
define("PathToCurrentPage", "/trans/");
define("FileName", "t_vat_setllement_edit_ro.php");
...
//Class_Initialize Event @2-A8BD98CD
    function clsGridt_vat_setllementGrid($RelativePath, & $Parent)
    {
        ...
        $this->DLink->Page = "t_vat_setllement_edit_ro.php";
...
//Show Method @2-BB672502
    function Show()
    {
        ...
        $this->DataSource->Parameters["expr261"] = 2;
...
//Update Method @23-A9AE2A12
    function Update()
    {
        ...
        $this->cp["start_period"] = new clsSQLParameter("ctrlstart_period", ccsText, "", "", $this->start_period->GetValue(true), "", false, $this->ErrorBlock);
        $this->cp["end_period"] = new clsSQLParameter("ctrlend_period", ccsText, "", "", $this->end_period->GetValue(true), "", false, $this->ErrorBlock);
        ...
        $this->cp["debt_vat_amt"] = new clsSQLParameter("ctrldebt_vat_amt", ccsFloat, "", "", $this->debt_vat_amt->GetValue(true), 0, false, $this->ErrorBlock);
        $this->cp["cr_adjustment"] = new clsSQLParameter("ctrlcr_adjustment", ccsFloat, "", "", $this->cr_adjustment->GetValue(true), 0, false, $this->ErrorBlock);
        $this->cp["cr_payment"] = new clsSQLParameter("ctrlcr_payment", ccsFloat, "", "", $this->cr_payment->GetValue(true), 0, false, $this->ErrorBlock);
        $this->cp["cr_others"] = new clsSQLParameter("ctrlcr_others", ccsFloat, "", "", $this->cr_others->GetValue(true), 0, false, $this->ErrorBlock);
        $this->cp["cr_stp"] = new clsSQLParameter("ctrlcr_stp", ccsFloat, "", "", $this->cr_stp->GetValue(true), 0, false, $this->ErrorBlock);
        $this->cp["db_interest_charge"] = new clsSQLParameter("ctrldb_interest_charge", ccsFloat, "", "", $this->db_interest_charge->GetValue(true), 0, false, $this->ErrorBlock);
        $this->cp["db_increasing_charge"] = new clsSQLParameter("ctrldb_increasing_charge", ccsFloat, "", "", $this->db_increasing_charge->GetValue(true), 0, false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["start_period"]->GetValue()) and !strlen($this->cp["start_period"]->GetText()) and !is_bool($this->cp["start_period"]->GetValue())) 
            $this->cp["start_period"]->SetValue($this->start_period->GetValue(true));
        if (!is_null($this->cp["end_period"]->GetValue()) and !strlen($this->cp["end_period"]->GetText()) and !is_bool($this->cp["end_period"]->GetValue())) 
            $this->cp["end_period"]->SetValue($this->end_period->GetValue(true));
        ...
        if (!is_null($this->cp["debt_vat_amt"]->GetValue()) and !strlen($this->cp["debt_vat_amt"]->GetText()) and !is_bool($this->cp["debt_vat_amt"]->GetValue())) 
            $this->cp["debt_vat_amt"]->SetValue($this->debt_vat_amt->GetValue(true));
        if (!strlen($this->cp["debt_vat_amt"]->GetText()) and !is_bool($this->cp["debt_vat_amt"]->GetValue(true))) 
            $this->cp["debt_vat_amt"]->SetText(0);
        if (!is_null($this->cp["cr_adjustment"]->GetValue()) and !strlen($this->cp["cr_adjustment"]->GetText()) and !is_bool($this->cp["cr_adjustment"]->GetValue())) 
            $this->cp["cr_adjustment"]->SetValue($this->cr_adjustment->GetValue(true));
        if (!strlen($this->cp["cr_adjustment"]->GetText()) and !is_bool($this->cp["cr_adjustment"]->GetValue(true))) 
            $this->cp["cr_adjustment"]->SetText(0);
        if (!is_null($this->cp["cr_payment"]->GetValue()) and !strlen($this->cp["cr_payment"]->GetText()) and !is_bool($this->cp["cr_payment"]->GetValue())) 
            $this->cp["cr_payment"]->SetValue($this->cr_payment->GetValue(true));
        if (!strlen($this->cp["cr_payment"]->GetText()) and !is_bool($this->cp["cr_payment"]->GetValue(true))) 
            $this->cp["cr_payment"]->SetText(0);
        if (!is_null($this->cp["cr_others"]->GetValue()) and !strlen($this->cp["cr_others"]->GetText()) and !is_bool($this->cp["cr_others"]->GetValue())) 
            $this->cp["cr_others"]->SetValue($this->cr_others->GetValue(true));
        if (!strlen($this->cp["cr_others"]->GetText()) and !is_bool($this->cp["cr_others"]->GetValue(true))) 
            $this->cp["cr_others"]->SetText(0);
        if (!is_null($this->cp["cr_stp"]->GetValue()) and !strlen($this->cp["cr_stp"]->GetText()) and !is_bool($this->cp["cr_stp"]->GetValue())) 
            $this->cp["cr_stp"]->SetValue($this->cr_stp->GetValue(true));
        if (!strlen($this->cp["cr_stp"]->GetText()) and !is_bool($this->cp["cr_stp"]->GetValue(true))) 
            $this->cp["cr_stp"]->SetText(0);
        if (!is_null($this->cp["db_interest_charge"]->GetValue()) and !strlen($this->cp["db_interest_charge"]->GetText()) and !is_bool($this->cp["db_interest_charge"]->GetValue())) 
            $this->cp["db_interest_charge"]->SetValue($this->db_interest_charge->GetValue(true));
        if (!strlen($this->cp["db_interest_charge"]->GetText()) and !is_bool($this->cp["db_interest_charge"]->GetValue(true))) 
            $this->cp["db_interest_charge"]->SetText(0);
        if (!is_null($this->cp["db_increasing_charge"]->GetValue()) and !strlen($this->cp["db_increasing_charge"]->GetText()) and !is_bool($this->cp["db_increasing_charge"]->GetValue())) 
            $this->cp["db_increasing_charge"]->SetValue($this->db_increasing_charge->GetValue(true));
        if (!strlen($this->cp["db_increasing_charge"]->GetText()) and !is_bool($this->cp["db_increasing_charge"]->GetValue(true))) 
            $this->cp["db_increasing_charge"]->SetText(0);
        ...
        "start_period = to_date('" . $this->SQLValue($this->cp["start_period"]->GetDBValue(), ccsText) . "','DD-MON-YYYY'),\n" .
        "end_period = to_date('" . $this->SQLValue($this->cp["end_period"]->GetDBValue(), ccsText) . "','DD-MON-YYYY'),\n" .
        "total_trans_amount = " . $this->SQLValue($this->cp["total_trans_amount"]->GetDBValue(), ccsFloat) . ",\n" .
        "total_vat_amount = " . $this->SQLValue($this->cp["total_vat_amount"]->GetDBValue(), ccsFloat) . ",\n" .
        "debt_vat_amt = " . $this->SQLValue($this->cp["debt_vat_amt"]->GetDBValue(), ccsFloat) . ",\n" .
        "cr_adjustment = " . $this->SQLValue($this->cp["cr_adjustment"]->GetDBValue(), ccsFloat) . ",\n" .
        "cr_payment = " . $this->SQLValue($this->cp["cr_payment"]->GetDBValue(), ccsFloat) . ",\n" .
        "cr_others = " . $this->SQLValue($this->cp["cr_others"]->GetDBValue(), ccsFloat) . ",\n" .
        "cr_stp = " . $this->SQLValue($this->cp["cr_stp"]->GetDBValue(), ccsFloat) . ",\n" .
        "db_interest_charge = " . $this->SQLValue($this->cp["db_interest_charge"]->GetDBValue(), ccsFloat) . ",\n" .
        "db_increasing_charge = " . $this->SQLValue($this->cp["db_increasing_charge"]->GetDBValue(), ccsFloat) . "\n" .
...
//Operation Method @3-9521B572
    function Operation()
    {
        ...
        $Redirect = "t_vat_setllement_edit_ro.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "t_vat_setllement_edit_ro.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "t_vat_setllement_edit_ro.html";
$BlockToParse = "main";
...
//Include events file @1-3759DABC
include_once("./t_vat_setllement_edit_ro_events.php");
