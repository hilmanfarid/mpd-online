...
//Page_BeforeShow @1-54DE2625
function Page_BeforeShow(& $sender)
{
    $Page_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $t_laporan_rekap_bphtb_hasil_verifikasi; //Compatibility
//End Page_BeforeShow
...
function tampilkan_html($param_arr){
	...
	$query="SELECT
			...
			AND reg_bphtb.status_verifikasi = 'Y'";
	$query.=$whereClause;
	...
	foreach($items as $item) {
		$output .= '<tr>';
			...
			$output .= '<td><button class="btn_tambah" onclick="updateStatusVerifikasi('.$item['t_bphtb_registration_id'].')">Cetak</button></td>';
		$output .= '</tr>';
