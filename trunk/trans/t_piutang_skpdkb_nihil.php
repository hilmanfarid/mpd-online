<?php 
//Include Common Files @1-C146EAC1
define("RelativePath", "..");
define("PathToCurrentPage", "/trans/");
define("FileName", "t_piutang_skpdkb_nihil.php");
...
//Class_Initialize Event @2-2B2E6BF8
    function clsGridt_vat_setllementGrid($RelativePath, & $Parent)
    {
        ...
        $this->DLink->Page = "t_piutang_skpdkb_nihil.php";
        $this->no_kohir = & new clsControl(ccsLabel, "no_kohir", "no_kohir", ccsText, "", CCGetRequestParam("no_kohir", ccsGet, NULL), $this);
        $this->nilai_piutang = & new clsControl(ccsLabel, "nilai_piutang", "nilai_piutang", ccsFloat, array(False, 0, Null, "", False, "", "", 1, True, ""), CCGetRequestParam("nilai_piutang", ccsGet, NULL), $this);
        $this->realisasi_piutang = & new clsControl(ccsLabel, "realisasi_piutang", "realisasi_piutang", ccsFloat, array(False, 0, Null, "", False, "", "", 1, True, ""), CCGetRequestParam("realisasi_piutang", ccsGet, NULL), $this);
        ...
        $this->t_piutang_pajak_penetapan_final_id = & new clsControl(ccsHidden, "t_piutang_pajak_penetapan_final_id", "t_piutang_pajak_penetapan_final_id", ccsFloat, "", CCGetRequestParam("t_piutang_pajak_penetapan_final_id", ccsGet, NULL), $this);
...
//Show Method @2-077FC435
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            $this->ControlsVisible["nilai_piutang"] = $this->nilai_piutang->Visible;
            $this->ControlsVisible["realisasi_piutang"] = $this->realisasi_piutang->Visible;
            $this->ControlsVisible["finance_period_code"] = $this->finance_period_code->Visible;
            $this->ControlsVisible["t_piutang_pajak_penetapan_final_id"] = $this->t_piutang_pajak_penetapan_final_id->Visible;
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->DLink->Parameters = CCAddParam($this->DLink->Parameters, "t_piutang_pajak_penetapan_final_id", $this->DataSource->f("t_piutang_pajak_penetapan_final_id"));
...
//GetErrors Method @2-4716B833
    function GetErrors()
    {
        $errors = "";
        $errors = ComposeStrings($errors, $this->DLink->Errors->ToString());
        $errors = ComposeStrings($errors, $this->no_kohir->Errors->ToString());
        $errors = ComposeStrings($errors, $this->nilai_piutang->Errors->ToString());
        $errors = ComposeStrings($errors, $this->realisasi_piutang->Errors->ToString());
        $errors = ComposeStrings($errors, $this->finance_period_code->Errors->ToString());
        $errors = ComposeStrings($errors, $this->t_piutang_pajak_penetapan_final_id->Errors->ToString());
...
//DataSource Variables @2-406357B3
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-ED552F7E
    function clst_vat_setllementGridDataSource(& $Parent)
    {
        ...
        $this->nilai_piutang = new clsField("nilai_piutang", ccsFloat, "");
        
        $this->realisasi_piutang = new clsField("realisasi_piutang", ccsFloat, "");
        
        $this->finance_period_code = new clsField("finance_period_code", ccsText, "");
        
        $this->t_piutang_pajak_penetapan_final_id = new clsField("t_piutang_pajak_penetapan_final_id", ccsFloat, "");
...
//SetOrder Method @2-EC4F9FEC
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "x.npwd , a.start_date";
...
//Open Method @2-7A115952
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT a.code as finance_period_code, nvl(realisasi_piutang,0) as realisasi_piutang, npwd, no_kohir,nilai_piutang,t_piutang_pajak_penetapan_final_id\n" .
        "FROM t_piutang_pajak_penetapan_final_2 x\n" .
        "left join p_finance_period a on a.p_finance_period_id = x.p_finance_period_id\n" .
        "WHERE ( upper(x.npwd) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%'\n" .
        "OR upper(a.code) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%' )) cnt";
        $this->SQL = "SELECT a.code as finance_period_code, nvl(realisasi_piutang,0) as realisasi_piutang, npwd, no_kohir,nilai_piutang,t_piutang_pajak_penetapan_final_id\n" .
        "FROM t_piutang_pajak_penetapan_final_2 x\n" .
        "left join p_finance_period a on a.p_finance_period_id = x.p_finance_period_id\n" .
        "WHERE ( upper(x.npwd) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%'\n" .
        "OR upper(a.code) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%' ){SQL_OrderBy}";
...
//SetValues Method @2-98788AFA
    function SetValues()
    {
        $this->no_kohir->SetDBValue($this->f("no_kohir"));
        $this->nilai_piutang->SetDBValue(trim($this->f("nilai_piutang")));
        $this->realisasi_piutang->SetDBValue(trim($this->f("realisasi_piutang")));
        $this->finance_period_code->SetDBValue($this->f("finance_period_code"));
        $this->t_piutang_pajak_penetapan_final_id->SetDBValue(trim($this->f("t_piutang_pajak_penetapan_final_id")));
...
//Class_Initialize Event @23-FA910A63
    function clsRecordt_vat_setllementForm($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->nilai_piutang = & new clsControl(ccsTextBox, "nilai_piutang", "nilai_piutang", ccsFloat, array(False, 2, Null, Null, False, "", "", 1, True, ""), CCGetRequestParam("nilai_piutang", $Method, NULL), $this);
            $this->nilai_piutang->Required = true;
            ...
            $this->masa_pajak = & new clsControl(ccsTextBox, "masa_pajak", "Masa Pajak", ccsText, "", CCGetRequestParam("masa_pajak", $Method, NULL), $this);
            $this->masa_pajak->Required = true;
            ...
            $this->t_piutang_pajak_penetapan_final_id = & new clsControl(ccsHidden, "t_piutang_pajak_penetapan_final_id", "t_piutang_pajak_penetapan_final_id", ccsFloat, "", CCGetRequestParam("t_piutang_pajak_penetapan_final_id", $Method, NULL), $this);
            if(!$this->FormSubmitted) {
                ...
                if(!is_array($this->nilai_piutang->Value) && !strlen($this->nilai_piutang->Value) && $this->nilai_piutang->Value !== false)
                    $this->nilai_piutang->SetText(0);
                if(!is_array($this->masa_pajak->Value) && !strlen($this->masa_pajak->Value) && $this->masa_pajak->Value !== false)
                    $this->masa_pajak->SetText(date("d-M-Y"));
...
//CheckErrors Method @23-98AE1559
...
//DataSource Variables @23-C91EC42A
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @23-B4A51BEE
...
//Prepare Method @23-1340E605
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlt_piutang_pajak_penetapan_final_id", ccsInteger, "", "", $this->Parameters["urlt_piutang_pajak_penetapan_final_id"], 0, false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
    }
//End Prepare Method

//Open Method @23-E0232C11
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT c.vat_code as jenis_pajak,d.code as finance_period_code,\n" .
        ...
        "where t_piutang_pajak_penetapan_final_id = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsInteger) . " ";
        $this->Order = "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->PageSize = 1;
        $this->query($this->OptimizeSQL(CCBuildSQL($this->SQL, $this->Where, $this->Order)));
...
//Operation Method @3-425F795D
    function Operation()
    {
        ...
        $Redirect = "t_piutang_skpdkb_nihil.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "t_piutang_skpdkb_nihil.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "t_piutang_skpdkb_nihil.html";
$BlockToParse = "main";
...
//Include events file @1-F99A5768
include_once("./t_piutang_skpdkb_nihil_events.php");
