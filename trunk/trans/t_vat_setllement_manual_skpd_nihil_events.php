...
// -------------------------
	...
	$sql = "select * from f_vat_settlement_manual_skpd_nihil($cusAccId,$Period,'$npwd','$ms_start','$ms_end',$kamar,$tot,$p_vat_type_dtl_id,$p_vat_type_dtl_cls_id,'$User')";
	//echo $sql;exit;
	//die($sql);
	$dbConn->query($sql);
	$dbConn->next_record();
	...
	if(!$cust_id == 0 && !$cust_id == ""){
		...
		//echo "window.open('http://192.168.0.112/mpd/report/cetak_formulir_skpd_nihil.php?t_vat_setllement_id=".$t_vat_setllement_id."','No Payment', 'left=0,top=0,width=500,height=500,toolbar=no,scrollbars=yes,resizable=yes')";
		echo '</script>';
...
//Page_OnInitializeView @1-F4CCD059
function Page_OnInitializeView(& $sender)
{
    $Page_OnInitializeView = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $t_vat_setllement_manual_skpd_nihil; //Compatibility
