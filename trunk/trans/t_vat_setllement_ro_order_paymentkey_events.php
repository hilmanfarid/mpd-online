...
// -------------------------
	...
	$t_vat_setllementForm->total_total->SetValue($totaltotal);
  	...
  	if($action_button=='flag_payment' && $action_button2!='cetak_register'){
		...
		echo "
  			<script>
  				window.open('../report/cetak_registrasi_payment_large.php?t_customer_order_id=".CCGetFromGet('t_customer_order_id')."&codeline=".$dbConn2->f('f_print_register')."', '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');
  			</script>
  		";
  		$dbConn2->close();
