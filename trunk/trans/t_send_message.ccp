<Page id="1" templateExtension="html" relativePath=".." fullRelativePath=".\trans" secured="False" urlType="Relative" isIncluded="False" SSLAccess="False" isService="False" cachingEnabled="False" validateRequest="True" cachingDuration="1 minutes" wizardTheme="sikm" wizardThemeVersion="3.0" needGeneration="0" pasteActions="pasteActions">
	<Components>
		<Record id="3" sourceType="SQL" urlType="Relative" secured="False" allowInsert="True" allowUpdate="True" allowDelete="False" validateData="True" preserveParameters="None" returnValueType="Number" returnValueTypeForDelete="Number" returnValueTypeForInsert="Number" returnValueTypeForUpdate="Number" name="t_cust_acc_status_editForm" returnPage="t_send_message.ccp" PathID="t_cust_acc_status_editForm" connection="ConnSIKP" pasteActions="pasteActions" parameterTypeListName="ParameterTypeList" activeCollection="ISQLParameters" dataSource="select * from t_cust_account where t_cust_account_id = {t_cust_account_id}" customUpdateType="SQL" customUpdate="select f_update_acc_status({t_cust_account_id},{p_account_status_id},'{description}','{valid_to}', '{user_name}')" pasteAsReplace="pasteAsReplace" customInsertType="SQL" customInsert="SELECT f_send_message_to_wp({sender_message_id},'{user_name}','{message_body}',null) as pesan">
			<Components>
				...
				<Hidden id="46" fieldSourceType="DBColumn" dataType="Text" name="t_message_inbox_id" wizardTheme="None" wizardThemeType="File" wizardThemeVersion="3.0" PathID="t_cust_acc_status_editFormt_message_inbox_id" fieldSource="t_message_inbox_id">
					<Components/>
					<Events/>
					<Attributes/>
					<Features/>
...
<Action actionName="Custom Code" actionCategory="General" id="51"/>
			...
			<ISPParameters/>
			<ISQLParameters>
				<SQLParameter id="48" variable="sender_message_id" parameterType="Control" dataType="Float" parameterSource="t_message_inbox_id" defaultValue="0"/>
...
<SQLParameter id="50" variable="message_body" parameterType="Control" dataType="Text" parameterSource="message_body"/>
	...
	<CodeFiles>
		<CodeFile id="Events" language="PHPTemplates" name="t_send_message_events.php" forShow="False" comment="//" codePage="windows-1252"/>
		<CodeFile id="Code" language="PHPTemplates" name="t_send_message.php" forShow="True" url="t_send_message.php" comment="//" codePage="windows-1252"/>
	</CodeFiles>
	<SecurityGroups/>
	<CachingParameters/>
	<Attributes/>
