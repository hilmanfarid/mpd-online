...
//Page_OnInitializeView @1-C035FF3C
function Page_OnInitializeView(& $sender)
{
    $Page_OnInitializeView = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $t_target_realisasi_jenis2; //Compatibility
...
//Page_BeforeInitialize @1-8FEEA1E8
...
//per_pajak Initialization @696-7EF2BFB5
    if ('t_target_realisasi_jenis2per_pajak' == CCGetParam('callbackControl')) {
        global $CCSLocales;
        $Service = new Service();
        $formatter = new TemplateFormatter();
        $formatter->SetTemplate(file_get_contents(RelativePath . "/trans/" . "t_target_realisasi_jenis2per_pajak.xml"));
...
//per_tahun Initialization @764-4FE23A12
    if ('t_target_realisasi_jenis2per_tahun' == CCGetParam('callbackControl')) {
        global $CCSLocales;
        $Service = new Service();
        $formatter = new TemplateFormatter();
        $formatter->SetTemplate(file_get_contents(RelativePath . "/trans/" . "t_target_realisasi_jenis2per_tahun.xml"));
        $Service->SetFormatter($formatter);
