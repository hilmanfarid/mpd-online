<?php
//Include Common Files @1-1B724D7F
define("RelativePath", "..");
define("PathToCurrentPage", "/param/");
define("FileName", "data_potensi_hotel_srvc_edit.php");
...
class clsRecordt_vat_reg_dtl_hotel_srvcForm { //t_vat_reg_dtl_hotel_srvcForm Class @94-2146E6BA
...
//Class_Initialize Event @94-0AF41742
    function clsRecordt_vat_reg_dtl_hotel_srvcForm($RelativePath, & $Parent)
    {
        ...
        $this->ErrorBlock = "Record t_vat_reg_dtl_hotel_srvcForm/Error";
        $this->DataSource = new clst_vat_reg_dtl_hotel_srvcFormDataSource($this);
        ...
        if($this->Visible)
        {
            $this->ComponentName = "t_vat_reg_dtl_hotel_srvcForm";
            ...
            $this->t_cacc_dtl_hotel_srvc_id = & new clsControl(ccsHidden, "t_cacc_dtl_hotel_srvc_id", "Id", ccsFloat, "", CCGetRequestParam("t_cacc_dtl_hotel_srvc_id", $Method, NULL), $this);
            ...
            $this->services = & new clsControl(ccsTextBox, "services", "Fasilitas Hotel", ccsText, "", CCGetRequestParam("services", $Method, NULL), $this);
            ...
            $this->DatePicker_valid_from = & new clsDatePicker("DatePicker_valid_from", "t_vat_reg_dtl_hotel_srvcForm", "valid_from", $this);
            $this->valid_to = & new clsControl(ccsTextBox, "valid_to", "Valid To", ccsText, "", CCGetRequestParam("valid_to", $Method, NULL), $this);
            $this->DatePicker_valid_to = & new clsDatePicker("DatePicker_valid_to", "t_vat_reg_dtl_hotel_srvcForm", "valid_to", $this);
...
//Initialize Method @94-0C7ACB4A
    function Initialize()
    {
        ...
        $this->DataSource->Parameters["urlt_cacc_dtl_hotel_srvc_id"] = CCGetFromGet("t_cacc_dtl_hotel_srvc_id", NULL);
    }
//End Initialize Method

//Validate Method @94-1BFFBF36
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        $Where = "";
        $Validation = ($this->t_cacc_dtl_hotel_srvc_id->Validate() && $Validation);
        ...
        $Validation = ($this->services->Validate() && $Validation);
        ...
        $Validation =  $Validation && ($this->t_cacc_dtl_hotel_srvc_id->Errors->Count() == 0);
        ...
        $Validation =  $Validation && ($this->services->Errors->Count() == 0);
...
//CheckErrors Method @94-794FABEC
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->t_cacc_dtl_hotel_srvc_id->Errors->Count());
        ...
        $errors = ($errors || $this->services->Errors->Count());
...
//InsertRow Method @94-9AA7D609
    function InsertRow()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeInsert", $this);
        if(!$this->InsertAllowed) return false;
        $this->DataSource->description->SetValue($this->description->GetValue(true));
        $this->DataSource->services->SetValue($this->services->GetValue(true));
...
//UpdateRow Method @94-D604178E
    function UpdateRow()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeUpdate", $this);
        if(!$this->UpdateAllowed) return false;
        $this->DataSource->t_cacc_dtl_hotel_srvc_id->SetValue($this->t_cacc_dtl_hotel_srvc_id->GetValue(true));
        $this->DataSource->description->SetValue($this->description->GetValue(true));
        $this->DataSource->services->SetValue($this->services->GetValue(true));
        $this->DataSource->t_cust_account_id->SetValue($this->t_cust_account_id->GetValue(true));
        $this->DataSource->valid_from->SetValue($this->valid_from->GetValue(true));
...
//Show Method @94-14B5A4D9
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            $Error = "";
            $Error = ComposeStrings($Error, $this->t_cacc_dtl_hotel_srvc_id->Errors->ToString());
            ...
            $Error = ComposeStrings($Error, $this->services->Errors->ToString());
...
} //End t_vat_reg_dtl_hotel_srvcForm Class @94-FCB6E20C

class clst_vat_reg_dtl_hotel_srvcFormDataSource extends clsDBConnSIKP {  //t_vat_reg_dtl_hotel_srvcFormDataSource Class @94-256B1BC3

//DataSource Variables @94-A69B3E98
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @94-5F2424D6
    function clst_vat_reg_dtl_hotel_srvcFormDataSource(& $Parent)
    {
        $this->Parent = & $Parent;
        $this->ErrorBlock = "Record t_vat_reg_dtl_hotel_srvcForm/Error";
        $this->Initialize();
...
//Prepare Method @94-DC25F9B7
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlt_cacc_dtl_hotel_srvc_id", ccsFloat, "", "", $this->Parameters["urlt_cacc_dtl_hotel_srvc_id"], "", false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
    }
//End Prepare Method

//Open Method @94-EC01406A
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT t_cacc_dtl_hotel_srvc_id, t_cust_account_id, services, \n" .
        ...
        "WHERE t_cacc_dtl_hotel_srvc_id = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsFloat) . " ";
        $this->Order = "";
...
//Insert Method @94-61D9C1B8
    function Insert()
    {
        ...
        $this->cp["created_by"] = new clsSQLParameter("expr840", ccsText, "", "", CCGetUserLogin(), "", false, $this->ErrorBlock);
        $this->cp["updated_by"] = new clsSQLParameter("expr841", ccsText, "", "", CCGetUserLogin(), "", false, $this->ErrorBlock);
        $this->cp["services"] = new clsSQLParameter("ctrlservices", ccsText, "", "", $this->services->GetValue(true), "", false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["services"]->GetValue()) and !strlen($this->cp["services"]->GetText()) and !is_bool($this->cp["services"]->GetValue())) 
            $this->cp["services"]->SetValue($this->services->GetValue(true));
        ...
        $this->SQL = "INSERT INTO t_cacc_dtl_hotel_srvc(t_cacc_dtl_hotel_srvc_id, description, created_by, updated_by, creation_date, updated_date, services, t_cust_account_id, valid_from, valid_to) \n" .
        "VALUES(generate_id('sikp','t_cacc_dtl_hotel_srvc','t_cacc_dtl_hotel_srvc_id'), '" . $this->SQLValue($this->cp["description"]->GetDBValue(), ccsText) . "', '" . $this->SQLValue($this->cp["created_by"]->GetDBValue(), ccsText) . "', '" . $this->SQLValue($this->cp["updated_by"]->GetDBValue(), ccsText) . "', sysdate, sysdate, '" . $this->SQLValue($this->cp["services"]->GetDBValue(), ccsText) . "', " . $this->SQLValue($this->cp["t_cust_account_id"]->GetDBValue(), ccsFloat) . ", to_date('" . $this->SQLValue($this->cp["valid_from"]->GetDBValue(), ccsText) . "','DD-MON-YYYY'), case when '" . $this->SQLValue($this->cp["valid_to"]->GetDBValue(), ccsText) . "' = '' then null else to_date('" . $this->SQLValue($this->cp["valid_to"]->GetDBValue(), ccsText) . "','dd-mon-yyyy') end)";
...
//Update Method @94-85FA923C
    function Update()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->CmdExecution = true;
        $this->cp["t_cacc_dtl_hotel_srvc_id"] = new clsSQLParameter("ctrlt_cacc_dtl_hotel_srvc_id", ccsFloat, "", "", $this->t_cacc_dtl_hotel_srvc_id->GetValue(true), 0, false, $this->ErrorBlock);
        $this->cp["description"] = new clsSQLParameter("ctrldescription", ccsText, "", "", $this->description->GetValue(true), "", false, $this->ErrorBlock);
        ...
        if (!is_null($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetValue()) and !strlen($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetText()) and !is_bool($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetValue())) 
            $this->cp["t_cacc_dtl_hotel_srvc_id"]->SetValue($this->t_cacc_dtl_hotel_srvc_id->GetValue(true));
        if (!strlen($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetText()) and !is_bool($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetValue(true))) 
            $this->cp["t_cacc_dtl_hotel_srvc_id"]->SetText(0);
        if (!is_null($this->cp["description"]->GetValue()) and !strlen($this->cp["description"]->GetText()) and !is_bool($this->cp["description"]->GetValue())) 
            $this->cp["description"]->SetValue($this->description->GetValue(true));
        if (!is_null($this->cp["updated_by"]->GetValue()) and !strlen($this->cp["updated_by"]->GetText()) and !is_bool($this->cp["updated_by"]->GetValue())) 
            $this->cp["updated_by"]->SetValue(CCGetUserLogin());
        ...
        "WHERE t_cacc_dtl_hotel_srvc_id=" . $this->SQLValue($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetDBValue(), ccsFloat) . " ";
...
//Delete Method @94-CE48D08C
    function Delete()
    {
        ...
        if (!strlen($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetText()) and !is_bool($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetValue(true))) 
            $this->cp["t_cacc_dtl_hotel_srvc_id"]->SetText(0);
        $this->SQL = "DELETE FROM t_cacc_dtl_hotel_srvc\n" .
        "WHERE t_cacc_dtl_hotel_srvc_id = " . $this->SQLValue($this->cp["t_cacc_dtl_hotel_srvc_id"]->GetDBValue(), ccsFloat) . "";
...
} //End t_vat_reg_dtl_hotel_srvcFormDataSource Class @94-FCB6E20C

//Initialize Page @1-D5DE5335
// Variables
$FileName = "";
$Redirect = "";
$Tpl = "";
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "data_potensi_hotel_srvc_edit.html";
$BlockToParse = "main";
...
//Include events file @1-5941F4FB
include_once("./data_potensi_hotel_srvc_edit_events.php");
...
// Controls
$t_vat_reg_dtl_hotel_srvcForm = & new clsRecordt_vat_reg_dtl_hotel_srvcForm("", $MainPage);
$MainPage->t_vat_reg_dtl_hotel_srvcForm = & $t_vat_reg_dtl_hotel_srvcForm;
$t_vat_reg_dtl_hotel_srvcForm->Initialize();
...
//Execute Components @1-D7017487
$t_vat_reg_dtl_hotel_srvcForm->Operation();
//End Execute Components

//Go to destination page @1-1360CC26
if($Redirect)
{
    $CCSEventResult = CCGetEvent($CCSEvents, "BeforeUnload", $MainPage);
    $DBConnSIKP->close();
    header("Location: " . $Redirect);
    unset($t_vat_reg_dtl_hotel_srvcForm);
    unset($Tpl);
    exit;
}
//End Go to destination page

//Show Page @1-3CCD0F51
$t_vat_reg_dtl_hotel_srvcForm->Show();
$Tpl->block_path = "";
