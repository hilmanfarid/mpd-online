<?php
//Include Common Files @1-F15A16E0
define("RelativePath", "..");
define("PathToCurrentPage", "/param/");
define("FileName", "p_vat_group.php");
...
//Class_Initialize Event @2-1F2095F5
    function clsGridp_vat_groupGrid($RelativePath, & $Parent)
    {
        ...
        $this->DLink->Page = "p_vat_group.php";
        ...
        $this->Insert_Link->Page = "p_vat_group.php";
    ...
    function Show()
    {
        ...
        if (!$this->IsEmpty) {
            ...
            while ($this->ForceIteration || (($this->RowNumber < $this->PageSize) &&  ($this->HasRecord = $this->DataSource->has_next_record()))) {
                ...
                $this->DLink->Parameters = CCAddParam($this->DLink->Parameters, "p_vat_group_id", $this->DataSource->f("p_vat_group_id"));
...
//DataSource Variables @2-2B24A8BA
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-68CDE7AB
...
//SetOrder Method @2-AC69CC9B
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "p_vat_group_id";
...
//Open Method @2-E4113FFE
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT p_vat_group_id, group_code, description, creation_date, created_by, updated_date, updated_by\n" .
        "FROM p_vat_group\n" .
        "WHERE upper(group_code) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%'\n" .
        "OR upper(description) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') cnt";
        $this->SQL = "SELECT p_vat_group_id, group_code, description, creation_date, created_by, updated_date, updated_by\n" .
        "FROM p_vat_group\n" .
        "WHERE upper(group_code) LIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%'\n" .
...
//Class_Initialize Event @3-1CEA8AB2
...
//Operation Method @3-DE20BE0D
    function Operation()
    {
        ...
        $Redirect = "p_vat_group.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "p_vat_group.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
//Class_Initialize Event @23-C6262E19
    function clsRecordp_vat_groupForm($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->p_vat_group_id = & new clsControl(ccsHidden, "p_vat_group_id", "Id", ccsFloat, "", CCGetRequestParam("p_vat_group_id", $Method, NULL), $this);
            ...
            $this->group_code = & new clsControl(ccsTextBox, "group_code", "Kode", ccsText, "", CCGetRequestParam("group_code", $Method, NULL), $this);
...
//CheckErrors Method @23-D40C0427
    function CheckErrors()
    {
        $errors = false;
...
//DataSource Variables @23-F595C8B8
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @23-E8B14334
...
//Prepare Method @23-A0B7C4AB
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlp_vat_group_id", ccsFloat, "", "", $this->Parameters["urlp_vat_group_id"], 0, false);
        $this->AllParametersSet = $this->wp->AllParamsSet();
    }
//End Prepare Method

//Open Method @23-8760B7DD
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT p_vat_group_id, group_code, description, to_char(creation_date,'DD-MON-YYYY')as creation_date, created_by, to_char(updated_date,'DD-MON-YYYY')as updated_date, updated_by\n" .
        "FROM p_vat_group\n" .
        "WHERE p_vat_group_id = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsFloat) . " ";
        $this->Order = "";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->PageSize = 1;
        $this->query($this->OptimizeSQL(CCBuildSQL($this->SQL, $this->Where, $this->Order)));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "p_vat_group.html";
$BlockToParse = "main";
...
//Include events file @1-B02F7BF3
include_once("./p_vat_group_events.php");
