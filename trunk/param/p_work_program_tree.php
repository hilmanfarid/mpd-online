...
define("FileName", "p_work_program_tree.php");
...
<?php
$query = "select p_work_program_id , NVL (parent_id, 0) parent_id, "
		. "REPLACE(upper(code), '''', '`') code, "
		. "NVL (program_name, '0') program_name FROM p_work_program START WITH "
		. "parent_id IS NULL CONNECT BY PRIOR p_work_program_id  = parent_id";
        ...
        <tbody>
            <tr>
                <td class="CobaltHeaderTD" style="color: #696969;font-size: 11pt;font-weight: bold;font-family:Calibri,Arial,Tahoma;>
                    <span class="style1">&nbsp;<b>PROGRAM KERJA</b></span></td>
            </tr>
            <tr>
...
var TREE_ITEMS =
 [
  ['HOME (ROOT)', 'p_work_program.php?parent_id=0&retree=1'
<?php
	...
	while ($dbConn->next_record()) {
		if ($dbConn->f("p_work_program_id")!=$bdmnid) {
			$bdmnid=$dbConn->f("p_work_program_id");
			...
			$fileName = "p_work_program.php";
			$nplevel = $dbConn->f("p_work_program_id");
			echo "['" . strtoupper($dbConn->f("code")." ". $dbConn->f("program_name")) ;
			if ($fileName=="") {
				echo "',''";
			} else {
				echo "','p_work_program.php?retree=1&parent_id=".$dbConn->f("p_work_program_id")."'";
			}
		}
	}
	while ($level>0) {
