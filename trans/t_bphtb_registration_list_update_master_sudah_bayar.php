<?php
//Include Common Files @1-0B79FF5C
define("RelativePath", "..");
define("PathToCurrentPage", "/trans/");
define("FileName", "t_bphtb_registration_list_update_master_sudah_bayar.php");
...
//Class_Initialize Event @2-DD30355F
    function clsGridt_bphtb_registration_list($RelativePath, & $Parent)
    {
        ...
        $this->DLink->Page = "t_bphtb_registration_list_update_master_sudah_bayar.php";
        ...
        $this->registration_no = & new clsControl(ccsHidden, "registration_no", "registration_no", ccsText, "", CCGetRequestParam("registration_no", ccsGet, NULL), $this);
...
//DataSource Variables @2-A7B80BC7
    var $Parent = "";
    var $CCSEvents = "";
    var $CCSEventResult;
    var $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-0D8FCB59
...
//Open Method @2-1A25E18A
    function Open()
    {
        ...
        "WHERE regis.t_bphtb_registration_id IN (select t_bphtb_registration_id from t_payment_receipt_bphtb) AND\n" .
        "(cust_order.order_no ILIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' OR\n" .
        "regis.wp_name ILIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' OR\n" .
        "regis.registration_no ILIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%')) cnt";
        $this->SQL = "select cust_order.*,regis.* from t_bphtb_registration regis\n" .
        "LEFT JOIN t_customer_order cust_order on regis.t_customer_order_id = cust_order.t_customer_order_id\n" .
        "WHERE regis.t_bphtb_registration_id IN (select t_bphtb_registration_id from t_payment_receipt_bphtb) AND\n" .
        "(cust_order.order_no ILIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' OR\n" .
        "regis.wp_name ILIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%' OR\n" .
        "regis.registration_no ILIKE '%" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "%') {SQL_OrderBy}";
...
//Operation Method @3-1796CAC6
    function Operation()
    {
        ...
        $Redirect = "t_bphtb_registration_list_update_master_sudah_bayar.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "t_bphtb_registration_list_update_master_sudah_bayar.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "t_bphtb_registration_list_update_master_sudah_bayar.html";
$BlockToParse = "main";
...
//Include events file @1-3F55A4A1
include_once("./t_bphtb_registration_list_update_master_sudah_bayar_events.php");
