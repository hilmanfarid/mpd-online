...
//Page_OnInitializeView @1-929D64ED
function Page_OnInitializeView(& $sender)
{
    $Page_OnInitializeView = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $t_rep_bulanan_target_vs_realisasi_monitor_large; //Compatibility
...
function GetCetakGeneralHTML($param_arr) {
	...
	for($i = 0; $i < count($data['vat_code']); $i++) {
		...
		if($pajak_text == $data['pajak_text'][$i]) {
			...
			$output .= '<tr style="background:#498CD6;font-weight:bold;color:#FFFFFF;font-size:15px;">';
	...
	$output .= '<tr style="background:#000000;font-weight:bold;font-size:20px;color:#FFFFFF;">';
