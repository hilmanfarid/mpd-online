...
// -------------------------
	...
	$sql = "select * from f_vat_settlement_manual_skpdkb_jabatan($cusAccId,$Period,'$npwd','$ms_start','$ms_end',$kamar,$tot,$p_vat_type_dtl_id,$p_vat_type_dtl_cls_id,'$User')";
	//echo $sql;exit;
	//die($sql);
	$dbConn->query($sql);
	$dbConn->next_record();
	...
	if(!$cust_id == 0 && !$cust_id == ""){
		...
		echo '<script language="javascript">';
		echo "randomnumber = Math.floor((Math.random()*100)+1);";
		echo "window.open('../report/cetak_formulir_skpdkb_jabatan_barcode.php?t_vat_setllement_id=".$t_vat_setllement_id."',randomnumber, 'left=0,top=0,width=500,height=500,toolbar=no,scrollbars=yes,resizable=yes');";
		echo "randomnumber = Math.floor((Math.random()*100)+1);";
		echo "window.open('../report/cetak_formulir_skpdkb_jabatan_v2.php?t_vat_setllement_id=".$t_vat_setllement_id."',randomnumber, 'left=0,top=0,width=500,height=500,toolbar=no,scrollbars=yes,resizable=yes');";
		echo '</script>';
...
//Page_OnInitializeView @1-877609D7
function Page_OnInitializeView(& $sender)
{
    $Page_OnInitializeView = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $t_vat_setllement_manual_skpdkb_jabatan; //Compatibility
