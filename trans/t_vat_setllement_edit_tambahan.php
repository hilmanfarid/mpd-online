<?php 
//Include Common Files @1-5875D36B
define("RelativePath", "..");
define("PathToCurrentPage", "/trans/");
define("FileName", "t_vat_setllement_edit_tambahan.php");
...
//Class_Initialize Event @2-F2EFC645
    function clsGridt_vat_setllementGrid($RelativePath, & $Parent)
    {
        ...
        $this->DLink->Page = "t_vat_setllement_edit_tambahan.php";
...
//SetOrder Method @2-47F20595
    function SetOrder($SorterName, $SorterDirection)
    {
        $this->Order = "a.creation_date desc";
        $this->Order = CCGetOrder($this->Order, $SorterName, $SorterDirection, 
            "");
    }
//End SetOrder Method

//Prepare Method @2-B76DEDB4
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        ...
        $this->wp->AddParameter("3", "expr386", ccsFloat, "", "", $this->Parameters["expr386"], 0, false);
    }
//End Prepare Method

//Open Method @2-295C0BC3
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (select b.code as finance_period_code,d.order_no,a.* from t_vat_setllement a\n" .
        ...
        "and d.p_order_status_id = 1) cnt";
        $this->SQL = "select b.code as finance_period_code,d.order_no,a.* from t_vat_setllement a\n" .
        "left join p_finance_period b on b.p_finance_period_id = a.p_finance_period_id\n" .
        "left join t_cust_account c on c.t_cust_account_id= a.t_cust_account_id\n" .
        "left join t_customer_order d on d.t_customer_order_id = a.t_customer_order_id\n" .
        "where p_settlement_type_id = 6\n" .
        "and d.p_order_status_id = 1{SQL_OrderBy}";
...
//Class_Initialize Event @23-95997382
...
//Open Method @23-2869EA90
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "select c.wp_name,c.wp_address_name||' '||c.wp_address_no as wp_address_name ,\n" .
        ...
        "where a.t_vat_setllement_id = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsFloat) . "  {SQL_OrderBy}";
        $this->Order = "a.creation_date desc ";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        $this->PageSize = 1;
        $this->query($this->OptimizeSQL(CCBuildSQL($this->SQL, $this->Where, $this->Order)));
...
//SetValues Method @23-A06C9334
    function SetValues()
    {
        ...
        $this->due_date->SetDBValue($this->f("due_date_2"));
        ...
        $this->db_interest_charge->SetDBValue(trim($this->f("db_interest_charge_2")));
        $this->db_increasing_charge->SetDBValue(trim($this->f("db_increasing_charge_2")));
...
//Operation Method @3-5D3CB510
    function Operation()
    {
        ...
        $Redirect = "t_vat_setllement_edit_tambahan.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "t_vat_setllement_edit_tambahan.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "t_vat_setllement_edit_tambahan.html";
$BlockToParse = "main";
...
//Include events file @1-746609F6
include_once("./t_vat_setllement_edit_tambahan_events.php");
