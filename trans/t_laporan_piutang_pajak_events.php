...
function print_laporan($param_arr){
	...
	while($dbConn->next_record()){
		$items[]= $item = array(
						...
						"sisa" => $dbConn->f("sisa"),
						"sisa_revisi" => $dbConn->f("sisa_revisi"),
						"tgl_bayar" => $dbConn->f("tgl_bayar_formated"),
						"besarnya" => $dbConn->f("besarnya"),
		...
		$pdf->RowMultiBorderWithHeight(array(
									...
									'Rp ' . number_format($item["sisa_revisi"], 2, ',', '.'),
									$item["tgl_bayar"],
									'Rp ' . number_format($item["besarnya"], 2, ',', '.'),
									'Rp ' . number_format($item["sisa_piutang"], 2, ',', '.'),
									$item["keterangan"],
									$item["year_code"]),array('LTB','LTB','LTB','LTB','LTB','LTB','LTB','LTB','LTB','LTB','LTBR'),6);
