...
function create_info_ama($param_arr) {
	$dbConn = new clsDBConnSIKP();
	$SQL = "select replace(replace(f_monitor_tipro_bphtb('".$param_arr['date_start']."','".$param_arr['date_end']."'),'(\"',''),'\")','') from dual";	
		//die($SQL);
	$dbConn->query($SQL);
			  
	return create_table_grid($dbConn, "INFORMASI MONITORING BPHTB", "", "", "Y", "", "2");
}

function GetCetakHTML($param_arr) {

	return '<table class="grid-table-container" cellspacing="0" cellpadding="0" border="0">
	<tr>
	<td>'.create_info_ama($param_arr).'</td>
	</tr>
	</table>';
}
