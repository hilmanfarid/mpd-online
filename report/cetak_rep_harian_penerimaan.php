...
class FormCetak extends FPDF {
	...
	*/
	...
	function PageCetak($data, $user) {
		...
		//untuk subtotal
		$j_target = array();
		$j_sptpd_hi = array(); //SPTPD hari ini
		$j_sptpd_sd_hl = array(); //SPTPD hari lalu
		$j_sptpd_sd_hi = array();
		$j_sspd_hi = array();
		$j_sspd_sd_hl = array();
		$j_sspd_sd_hi = array();
		...
		for ($i = 0; $i < count($data["nama_ayat"]); $i++) {
			...
			$this->RowMultiBorderWithHeight(array($no,
											  $data["kode_jns_pajak"][$i] . "." . $data["type_ayat"][$i],
											  $data["nama_ayat"][$i],
											  ...
											  0,
											  0,
											  number_format($data["r_jml_hari_ini"][$i], 0, ',', '.'),
											  0,
											  number_format($data["r_jml_sd_hari_lalu"][$i], 0, ',', '.'),
											  0,
											  number_format($data["r_jml_sd_hari_ini"][$i], 0, ',', '.'),
											  0,
											  number_format(0, 0, ',', '.'),
											  number_format(0, 0, ',', '.'),
											  number_format(0, 0, ',', '.')
			...
			$j_sptpd_hi [] = $data["jml_hari_ini"][$i];
			$j_sptpd_sd_hl [] = $data["jml_sd_hari_lalu"][$i];
			$j_sptpd_sd_hi [] = $data["jml_sd_hari_ini"][$i];
			$j_sspd_hi [] = $data["r_jml_hari_ini"][$i];
			$j_sspd_sd_hl [] = $data["r_jml_sd_hari_lalu"][$i];
			$j_sspd_sd_hi [] = $data["r_jml_sd_hari_ini"][$i];
			...
			if($jenis != $jenissesudah){
				$this->Cell($ltable1 + $ltable5, $this->height, "JUMLAH " . strtoupper($data["nama_jns_pajak"][$i]), "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, array_sum($j_target), "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, array_sum($j_sptpd_hi), "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, array_sum($j_sptpd_sd_hl), "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, array_sum($j_sptpd_sd_hi), "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, array_sum($j_sspd_hi), "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, array_sum($j_sspd_sd_hl), "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, array_sum($j_sspd_sd_hi), "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, 0, "TBLR", 0, 'C');
				$this->Ln();
				
				$j_t_target += array_sum($j_target);
				$j_t_sptpd_hi += array_sum($j_sptpd_hi);
				$j_t_sptpd_sd_hl += array_sum($j_sptpd_sd_hl);
				$j_t_sptpd_sd_hi += array_sum($j_sptpd_sd_hi);
				$j_t_sspd_hi += array_sum($j_sspd_hi);
				$j_t_sspd_sd_hl += array_sum($j_sspd_sd_hl);
				$j_t_sspd_sd_hi += array_sum($j_sspd_sd_hi);
			}
			
			if($i == count($data['nama_ayat']) - 1){
				$this->Cell($ltable1 + $ltable5, $this->height, "JUMLAH TOTAL", "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, $j_t_target, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, $j_t_sptpd_hi, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, $j_t_sptpd_sd_hl, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, $j_t_sptpd_sd_hi, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, $j_t_sspd_hi, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, $j_t_sspd_sd_hl, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, $j_t_sspd_sd_hi, "TBLR", 0, 'C');
				$this->Cell($ltable1, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, 0, "TBLR", 0, 'C');
				$this->Cell($ltable3, $this->height, 0, "TBLR", 0, 'C');
