...
class FormCetak extends FPDF {
	...
	*/
	...
	function page_footer($data, $user){
		...
		$this->Cell($lowTable1, $this->height + 2, "", "LR", 0, 'C');
		if ($data["no_urut"] != "") {
			$this->Image('http://'.$_SERVER['HTTP_HOST'].'/mpd/include/qrcode/generate-qr.php?param='.$data["barcode"],90,$this->getY(),25,0,'PNG');
		}
		$this->Ln();
		$this->Cell($lowTable1, $this->height + 2, "", "LR", 0, 'C');
	...
	function PageCetak($data, $user) {
		...
		$this->Cell($ltable4, $this->height + 2, $total_jum, "TBLR", 0, 'R');
