...
define("FileName", "cetak_trans_harian_pdf.php");
include_once(RelativePath . "/Common.php");
include_once("../include/fpdf.php");

$t_cust_account_id = CCGetFromGet("t_cust_account_id", "");
$trans_date = CCGetFromGet("trans_date", "");
$data = array();
...
$query="select bill_no, service_desc, service_charge, vat_charge, description
from f_get_cust_acc_dtl_trans(".$t_cust_account_id.",'".$trans_date."')AS tbl (t_cust_acc_dtl_trans_id)";

$dbConn->query($query);
while ($dbConn->next_record()) {
		$data["bill_no"][] = $dbConn->f("bill_no");
		$data["service_desc"][] = $dbConn->f("service_desc");
...
class FormCetak extends FPDF {
	...
	function PageCetak($data) {
		...
		$kol4 = ($lengthCell * 3) / 15;
		$kol5 = ($lengthCell * 3) / 15; 
		
		$this->Cell($lengthCell, $this->height, "TRANSAKSI HARIAN WP", 0, 0, 'C');
		$this->Ln(10);
		$this->Cell($kol1, $this->height, "No. Faktur", 1, 0, 'C');
		$this->Cell($kol2, $this->height, "Nama Transaksi", 1, 0, 'C');
		$this->Cell($kol3, $this->height, "Nilai Transaksi", 1, 0, 'C');
		$this->Cell($kol4, $this->height, "Nilai Pajak", 1, 0, 'C');
		$this->Cell($kol5, $this->height, "Descripsi", 1, 0, 'C');
		$this->Ln();
		$this->SetWidths(array($kol1, $kol2, $kol3, $kol4, $kol5));
		$this->SetAligns(array("C", "L", "R", "R", "L"));
		for ($i=0; $i<count($data['bill_no']); $i++) {
		$this->RowMultiBorderWithHeight(array($data["bill_no"][$i],
											  $data["service_desc"][$i],
