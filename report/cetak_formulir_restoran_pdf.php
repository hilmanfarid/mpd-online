...
define("FileName", "cetak_formulir_restoran_pdf.php");
...
$query="begin;";
$dbConn->query($query);
$query="select * from f_cetak_pajak_restaurant(".$custId.", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "
...
$dbConn->query($query);
while ($dbConn->next_record()) {
	$dataBaru["service_type_desc"][] = $dbConn->f("service_type_desc");
	$dataBaru["seat_qty"][] = $dbConn->f("seat_qty");
	$dataBaru["table_qty"][] = $dbConn->f("table_qty");
	$dataBaru["max_service_qty"][] = $dbConn->f("max_service_qty");	
	$dataBaru["avg_subscription"][] = $dbConn->f("avg_subscription");
	$dataBaru["rest_service_type_code"][] = $dbConn->f("rest_service_type_code");
}
...
class FormCetak extends FPDF {
	...
	*/
	...
	function PageCetak($data,$dataArr,$dataBaru) {
		...
		$this->Ln();
		$this->Cell($lengthCell, $this->height, "WAJIB PAJAK RESTORAN", "LR", 0, 'C');
		$this->Ln();
		...
		*/
		...
		$this->Cell($per2, $this->height, "Pendataan Pajak Restoran", "TB", 0, 'L');
		$this->Cell($per3, $this->height, "", "TBR", 0, 'L');
		$this->Ln();
		...
		$this->Cell($kol6, $this->height*2, "", "TBLR", 0, 'C');
		$this->Ln($this->height-5);
		$this->Cell($kol1, $this->height*2, "", "L", 0, 'C');
		$this->Cell($kol2, $this->height*2, "Jenis Pelayanan", 0, 0, 'C');
		$this->Cell($kol3, $this->height*2, "Jumlah Kursi", 0, 0, 'C');
		$this->Cell($kol4, $this->height*2, "Jumlah Meja", 0, 0, 'C');
		$this->Cell($kol5, $this->height*2, "Daya Tampung", 0, 0, 'C');
		$this->Cell($kol6, $this->height+2, "Jumlah pengunjung", 0, 0, 'C');
		...
		$this->Cell($kol4, $this->height, "", 0, 0, 'C');
		$this->Cell($kol5, $this->height, "", 0, 0, 'C');
		$this->Cell($kol6, $this->height-2, "rata-rata per Bulan", 0, 0, 'C');
		...
		for ($i=0; $i<count($dataBaru['service_type_desc']); $i++) {
		$this->RowMultiBorderWithHeight(array("",
											  $dataBaru["service_type_desc"][$i],
											  $dataBaru["seat_qty"][$i],
											  $dataBaru["table_qty"][$i],
											  $dataBaru["max_service_qty"][$i],
											  $dataBaru["avg_subscription"][$i],
											  "")
											 ,
										array('L',
										      'TBL',
