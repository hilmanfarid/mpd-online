...
$result = $dbConn->query("SELECT count(*) as count FROM p_job_position WHERE code ilike '$searchTerm'");
$dbConn->next_record();
$row = $dbConn->Record;
$count = $row['count'];
...
if($total_pages!=0) $SQL = "SELECT * FROM p_job_position WHERE code ilike '$searchTerm' ORDER BY $sidx $sord OFFSET $start LIMIT $limit";
else $SQL = "SELECT * FROM p_job_position WHERE code ilike '$searchTerm'";
