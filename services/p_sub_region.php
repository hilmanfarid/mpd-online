...
$result = $dbConn->query("SELECT count(*) as count FROM p_region
						where parent_id=".$_GET['parent_id']." and region_name ilike '$searchTerm'");
$dbConn->next_record();
$row = $dbConn->Record;
$count = $row['count'];
...
if($total_pages!=0) $SQL = "SELECT * FROM p_region
						where parent_id=".$_GET['parent_id']." and region_name ilike '$searchTerm' ORDER BY $sidx $sord OFFSET $start LIMIT $limit";
