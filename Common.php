...
//Connection Settings @0-5B398AEC
...
//Initialize Common Variables @0-702FD302
...
//ConnSIKP Connection Class @-9CF38162
class clsDBConnSIKP extends DB_Adapter
{
    function clsDBConnSIKP()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["ConnSIKP"]);
        parent::Initialize();
...
function CCSecurityRedirect($GroupsAccess, $URL)
{
    ...
    {
        if(!strlen($URL))
            $Link = ServerURL . "main/sikp_login.php";
        else
            $Link = $URL;
...
//CCLoginUser @0-443E962E
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnSIKP();
    $SQL = "SELECT p_app_user_id, p_user_status_id, user_pwd FROM p_app_user WHERE app_user_name=" . $db->ToSQL($login, ccsText) . " AND user_pwd=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("p_app_user_id"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("p_user_status_id"));
    }
    return $Result;
}
//End CCLoginUser
